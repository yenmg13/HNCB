...
judgment_start_markers = ["上","上列","主文", "理由", "裁定如下", "判決如下",
    ...
    for idx, link in enumerate(url_elements, start=1):
        ...
        try:
            ...
            def extract_parties_from_abbr(soup, debug=False):
                  ...
                  paragraphs = soup.find_all("div", id=re.compile(r"^(pasted_)?paragraph_"))
                  merged_lines = []
                  buffer = ""
                  prev_text = ""
                  ...
                  for idx, div in enumerate(paragraphs):
                      ...
                      # Step 2️⃣ 轉為乾淨文字 + 修正中文字間空白
                      text = BeautifulSoup(html, "html.parser").get_text(" ", strip=True)
                      text = re.sub(r"\s+", " ", text).strip()
                      # 移除中文字之間的空白（代 理 人 → 代理人）
                      text = re.sub(r"(?<=[\u4e00-\u9fa5])\s+(?=[\u4e00-\u9fa5])", "", text)
                      if not text:
                          continue
                      ...
                      # Step 2.2 若上一行以「即」結尾且本行有角色 → 合併（例如：聲請人(即 債務人)）
                      if buffer and re.search(r"即\s*$", buffer) and any(role in text[:8] for role in role_set):
                          if debug:
                              print(f"  ↳ [MERGE 即結構-上即下角] {buffer[-10:]} + {text[:15]}")
                          buffer = buffer.rstrip("即").strip() + " 即 " + text
                          prev_text = text
                          continue

                      # Step 2.3 若本行以「即」開頭且後面有角色 → 合併到上一行（例如：上一行是『聲請人』，本行是『即 債務人』）
                      if text.strip().startswith("即") and any(role in text[:8] for role in role_set):
                          if debug:
                              print(f"  ↳ [MERGE 即結構-下即上角] {buffer[-10:]} + {text[:15]}")
                          # 把開頭「即」去掉避免重複
                          cleaned_text = re.sub(r"^即\s*", "", text)
                          buffer = buffer.strip() + " 即 " + cleaned_text
                          prev_text = text
                          continue

                      
                      # Step 3️⃣ 判斷主文 or 角色行
                  ...
                  # Step 5️⃣ 角色內容解析（長詞優先）
                  for line in merged_lines:
                      matched_role = None
                      for role in sorted(role_set, key=len, reverse=True):  # 長詞優先，法定代理人優先於代理人
                          if role in line:
                              matched_role = role
                              break
                      if not matched_role:
                      ...
                      if m and m.group(1) in role_set:
                          ...
                          if cleaned:
                              ...
                              if debug:
                                  print(f"  ↳ [即角色] {matched_role} ⇄ {alias_role} : {cleaned}")
                      else:
                          after = re.sub(r"\s+", " ", after).strip(" ,;；、")
                          result[matched_role].append(after)
                          if debug:
                              print(f"  ↳ [人名抽取] {matched_role}: {after}")
            ...
            soup = clean_html_before_extract(soup)   # ← 在 extract 前清除頁尾
            ...
            # 1) JSONL（全文/摘要）
            if OUTPUT_OPTIONS.get("JSONL" , False):
              append_jsonl({
                  "url": share_link,
              ...
              print_debug(f"已儲存JSONL：{JSONL_PATH}")
            # 2) 業管 CSV（精簡摘要）
